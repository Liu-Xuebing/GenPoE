...
class NQ_TQA_SQuAD_Dataset(Dataset):
    def __init__(self, config, tok, status):
        self.config = config
        self.tok = tok
        self.status = status
        self.data = []
        if status=='Train':
            if config.data_name == 'NQ':
                with open(config.NQ_train_dataset) as train_data:
                ...
                with open(config.TQA_train_dataset) as train_data:
                    datas = json.load(train_data)
                for data in datas:
                    for p in data['ctxs']:
                        if p['has_answer']:
...
def make_Training_loader(config, tok):
    train_set = NQ_TQA_SQuAD_Dataset(config, tok, status='Train')
...
def make_Validation_loader(config, tok):
    valid_set = NQ_TQA_SQuAD_Dataset(config, tok, status='Test')
